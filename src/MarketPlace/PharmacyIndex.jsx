...
export default function PharmacyIndex() {
  ...
  return (
    <div>
      <Row className="pharmacy_index pt-4 pb-4 m-0 p-0">
        <Col md={12}>
          <div className="container">
            ...
            <Row>
              {drugData.map((item) => (
                <Col lg={2}>
                  <Card
                    className="index_card shadow-sm p-2"
                    onClick={() => {
                      toggle()
                      setSelectItem(item)
                    }}
                  >
                    ...
                    {/* </CardBody> */}
                    <div>
                      <h5 className="drug_name">
                        {item.drugName.length > 14
                          ? `${item.drugName.substring(0, 14)}...`
                          : item.drugName}
                      </h5>
                      <p className="drug_generic_name">
                        {item.drugDescription.length > 20
                          ? `${item.drugDescription.substring(0, 20)}...`
                          : item.drugDescription}
                      </p>
                      <h5 className="drug_name">
                        {item.drugPrice} <span className="near">Near</span>
                      </h5>
                    </div>
      ...
      <Modal size="lg" isOpen={open} className="pharmacy_index_modal">
        <Row className="p-4">
          ...
          <Col md={7}>
            <h3 className="cart_drug_name">{selectItem.drugName}</h3>
            <p className="cart_drug_description">
              {selectItem.drugDescription}
            </p>
            <h3 className="cart_drug_price">
              {selectItem.drugPrice}
              <span className="cart_drug_currenct"> Near</span>
            </h3>
            ...
            <Button
              className={'pharmacy_btn add_to_cart_btn mt-3'}
              btnText={`Add to cart`}
              onClick={() => {
          ...
          <div className="p-4">
            <Row className="text-center">
              <p className="added_notificatio">
                A new drug has been added to your Shopping Cart.
              </p>
              <Col md={6}>
                <Button
